...
Cypress.Commands.add('forgotMyPasswordSubmit', (email) => {
  ...
  cy.textInput(input).get('#ForgotMyPasswordSubmit').click().end()
})

Cypress.Commands.add('clickOnForgotPasswordLink', (email) => {
  cy.get('#TextLink').contains('Forgot password?').click().end()
})

Cypress.Commands.add('backToLogin', (email) => {
  cy.get('#TextLink').contains('back').click().end()
})

Cypress.Commands.add('hasWarningMessage', () => {
  cy.get('.text-danger').contains('Could not sign you in').end()
})

Cypress.Commands.add('submitEmailToBeVerified', (email) => {
  cy.get('#EmailVerificationEmail')
    .type(email)
